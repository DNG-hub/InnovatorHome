...
export type Topic = {id:string;title:string;description:string;image:string;secondaryImage?:string;motif?:string};
...
export function coverFor(post:Translation) {
  ...
  return typeof path==='string' && /^\/images\/[a-zA-Z0-9/_-]+\.(webp|avif|png|jpg)$/.test(path) ? path : (post.data.project==='commons' ? topicFor(post)?.secondaryImage || topicFor(post)?.image : topicFor(post)?.image);
}
